(function() {
  var escapeRegex, fs, ned, nedCommands, nedReplace, nedSuppress, optimist, path, showHelpAndExit, usage;
  ...
  optimist = function(command, argv) {
    var opt;
    opt = require('../node_modules/optimist')(argv).usage(usage).boolean([1, 2, 3, 4, 5, 6, 7, 8, 9]);
    if (command === 'replace' || command === 'suppress') {
  ...
  ned = function(process) {
    var arg, argv, at, cmd, command, commands, file, inplace, input, neds, next, node, options, output_debug, _i, _len, _ref, _ref2, _results;
    ...
    if (next !== 'command') {
      showHelpAndExit(1);
      if (output_debug) {
        console.log("Expected 'command', got " + next);
      }
      process.exit(1);
    }
